    ...
    def train(self,epoch):
        ...
        for batch_idx, (inputs, targets) in enumerate(self.trainloader):
            ...
            if batch_idx ==0 and batch_idx%5==0:
                ...
                an = torch.cat((an, an_hard),0)
                se = torch.cat((se, se_hard), 0)
                pos = torch.cat((pos, pos_hard), 0)
                neg = torch.cat((neg, neg_hard), 0)
                # print(len(neg))
            ...
            # mainLoss = TwoLoss[0]
            # hardind = TwoLoss[1][:int(1/4*cfg['batch_size'])]

            tem_dict = nn.PairwiseDistance()(outputs[1], outputs[4]) - nn.PairwiseDistance()(outputs[1], outputs[3])
            # tem_dict += (nn.PairwiseDistance()(outputs[1], outputs[3]) - nn.PairwiseDistance()(outputs[1], outputs[2]))
            hardind = torch.sort(tem_dict)[1][:int(1/4*cfg['batch_size'])]
            # hardind = torch.sort(mainLoss,descending=True)[1][:int(1/4*cfg['batch_size'])]
            an_hard = an[hardind]
            se_hard = se[hardind]
            pos_hard = pos[hardind]
            neg_hard = neg[hardind]
        ...
        if (epoch+1)%10==0 or (epoch+1)==cfg['training']['max_epoch']:
...
if __name__=="__main__":
    ...
    # parser.add_argument('--beta', default=0.1, type=float, help='Weighting of Anchor loss component')
    # parser.add_argument('--lbda', default=0.2, type=float, help='Weighting of Uknown Repulse loss component')
    # parser.add_argument('--gamma', default=0.2, type=float, help='Weighting of Uknown Repulse loss component')
    # parser.add_argument('--m', default=10, type=float, help='Weighting of Uknown Repulse loss component')
    parser.add_argument('--tensorboard', '-t', default=False, action='store_true', help='Plot on tensorboardX')
    parser.add_argument('--name', default="HK_trip_tanhNet", type=str, help='Optional name for saving and tensorboard')
    ...
    parser.add_argument('--cs_num', default=-1, type=int, help='gpu number')
