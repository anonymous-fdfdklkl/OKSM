...
#
epoch = {'fiw-HK_trip_CNN_point_Net_7type':250,'fiw-HK_trip_CNN_Net_7type':200,'fiw-JLNet_7type':100,
         'fiw-close_7type':100,'fiw-class_7type':200,'fiw-cac_7type':150, 'fiw-HK_trip_tanhNet_7type':40,
         'fiw-HK_trip_tanhNet_7type_hard':40,'fiw-HK_trip_tanhNet_notanh':40,'fiw-HK_trip_tanhNet_different_7type':40,
         'fiw-FG_adam':10,'fiw-HK_trip_FG_adam':20
         }[N]
...
for trial_num in range(args.start_trial, args.start_trial + args.num_trials):
    ...
    knownloader, unknownloader, testloader  = fiw_loader(args, cfg).get_fiw_eval_loaders_img_new()
    name = args.model
    if args.real_sn:
        name += 'real'
    ...
    if "JLNet" in args.name:
        ...
        if "4_type" or '4type' in args.name:
    ...
    #### 1) known vs unknown
    auroc_knun, op_shr_knun = metrics.auroc(known_set, unk_set, in_low=inlow, show=False, sv_roc = True,name=[name,'known','unknown'],trial_num=trial_num)


    #### kin vs nonkin (within unknon sets)

    auroc_kinon, op_shr_kinnon = metrics.auroc(unk_kin_set, unk_nonkin_set, in_low=inlow, show=False, sv_roc = True,name=[name,'kin','non-kin'],trial_num=trial_num)
...
print('Primary:Precision:{:.4f}, Recall:{:.4f}, F1:{:.4f}'.format(np.mean(all_k_precision),np.mean(all_k_recall),np.mean(all_k_f1)))
print('Secondary:Precision:{:.4f}, Recall:{:.4f}, F1:{:.4f}'.format(np.mean(all_ukkin_precision),np.mean(all_ukkin_recall),np.mean(all_ukkin_f1)))
print('Non-kin:Precision:{:.4f}, Recall:{:.4f}, F1:{:.4f}'.format(np.mean(all_unk_precision),np.mean(all_unk_recall),np.mean(all_unk_f1)))
#### 1) known vs unknown
