...
from .datasets import kfw_caur, train_transform,test_transform,fiw_dataset,fiw_dataset_test,fiw_dataset_new,fiw_dataset_img_new
...
class fiw_loader():
    def __init__(self,options):
        ...
        # self.data_pth = '/var/scratch/wwang/DATA/FIW/rfiw2020/train-faces'
        # self.data_pth = '/var/scratch/wwang/DATA/FIW/rfiw2020-new/train-faces'
        # self.data_pth = '/var/scratch/wwang/DATA/FIW/origin/train-faces'
        self.data_pth ='/var/scratch/wwang/DATA/FIW/rfiw2020-new/train-faces'
        self.data_set = fiw_dataset
        print('old data settings, new imag path')
    ...
    def get_fiw_eval_loaders_new_get(self):
        # train_ls = self.train_ls
        train_ls_test = self.train_ls_test
        data_pth = self.data_pth
        opt = self.options
        test_set = fiw_dataset_new(train_ls_test, data_pth, opt['eval_thre'],
                            transform=test_transform,
                            real_sn=opt['real_sn'])
        known_set = fiw_dataset_new(train_ls_test, data_pth, opt['eval_thre'],
                             transform=test_transform,
                             real_sn=opt['real_sn'], get_pos=True)
        unknown_set = fiw_dataset_new(train_ls_test, data_pth, opt['eval_thre'],
                               transform=test_transform,
                               real_sn=opt['real_sn'], get_neg=True)
